...
DataValidationArtifact = namedtuple(
    "DataValidationArtifact", ["accepted_file_path", "rejected_dir"]
)
...
PartialModelTrainerRefArtifact = namedtuple(
    ...
    ["trained_model_file_path", "label_indexer_model_file_path"],
)

PartialModelTrainerMetricArtifact = namedtuple(
    "PartialModelTrainerMetricArtifact", ["f1_score", "precision_score", "recall_score"]
)
...
class ModelTrainerArtifact:
    ...
    def _asdict(self):  # sourcery skip: dict-literal, merge-dict-assign
        try:
            response = dict()
            response[
                "model_trainer_ref_artifact"
